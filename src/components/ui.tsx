...
const buttonBase =
  "focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-[4px] px-5 py-2.5 text-sm font-medium transition-[color,background-color,border-color,transform] duration-150 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 disabled:active:scale-100";
...
export function StatTile({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
    <Card className="border-x-0 border-b-0 p-4 sm:p-5">
      <div className="tracking-label text-[10px] font-semibold text-foreground-soft">{label}</div>
      <div className="mt-2 text-2xl font-medium tabular-nums tracking-[-0.045em] text-foreground">{value}</div>
